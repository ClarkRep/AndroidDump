...
import static com.clark.leaklib.Constants.Bytes.MB;
  ...
  public static class HeapThreshold {
    ...
    public static float getDefaultPercentRation() {
      ...
      if (Debug.VERBOSE_LOG) {
        Log.i("koom", "max mem " + maxMem);
      }
      if (maxMem >= VM_512_DEVICE) {
        return Constants.HeapThreshold.PERCENT_RATIO_IN_512_DEVICE;
      } else if (maxMem >= VM_256_DEVICE) {
        return Constants.HeapThreshold.PERCENT_RATIO_IN_256_DEVICE;
      } else if (maxMem >= VM_128_DEVICE) {
        return Constants.HeapThreshold.PERCENT_RATIO_IN_128_DEVICE;
      }
      return Constants.HeapThreshold.PERCENT_RATIO_IN_512_DEVICE;
    }

    public static float getDefaultMaxPercentRation() {
      return Constants.HeapThreshold.PERCENT_MAX_RATIO;
    }
